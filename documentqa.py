...
def generate_response(link, openai_api_key, query_text):
    ...
    llm = ChatOpenAI(model_name="gpt-3.5-turbo-0125", temperature=0, openai_api_key=openai_api_key) #Use the normal GPT-3.5 model for now
...
with st.form('myform', clear_on_submit=True):
    ...
    if submitted:
        with st.spinner('Calculating...'):
            response = generate_response(link, openai_api_key, query_text)
